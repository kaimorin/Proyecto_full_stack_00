...
class NotasControllerStandAloneTest {
 ...
 @BeforeEach
    void setup() {
        ...
        NotasController notasController = new NotasController(notasService, authService); // crea una instancia de NotasController pasando los mocks de NotasService y AuthService como dependencias
        mockMvc = MockMvcBuilders.standaloneSetup(notasController).build(); // configura MockMvc para usar la instancia de NotasController creada, lo que permite simular solicitudes HTTP a los endpoints del controlador durante las pruebas
    }
    ...
    void testUnauthorizedAccess() throws Exception { // prueba para verificar que el endpoint de listar autos requiere autenticación y devuelve un error 401 Unauthorized cuando se proporciona un token inválido
        mockMvc.perform(get("/api/v1/notas/list") // simula una solicitud GET al endpoint "/api/v1/notas/list" con un encabezado de autorización que contiene un token inválido
                .header("Authorization", "Bearer invalid")) // simula una solicitud GET al endpoint "/api/v1/notas/list" con un encabezado de autorización que contiene un token inválido
